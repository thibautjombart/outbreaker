...
% (the hyperparameters $\pi$ and $\phi$ are dropped as they only affect the priors on $\kappa$ and $\alpha$):
...
% & & \times \underbrace{p(\alpha_i | \phi) p(\kappa_i | \pi)}_{\Omega_i^3}
...
%  & & \times (\mathbf{1}_{\{ \alpha_i=0\}} \phi + \mathbf{1}_{\{ \alpha_i>0\}} (1-\phi)) \times \pi^{\kappa_i-1}
\end{eqnarray}
...
% \begin{eqnarray}
%  f_{\mathcal{NB}}(1 | \kappa_i-1, \pi) = \frac{(\kappa_i-1)!}{(\kappa_i-2)!} (1-\pi)^{\kappa_i-1} \pi
% \end{eqnarray}
...
%       \kappa, \mu_1, \gamma uniform
	...
	\item $p(\phi) = \beta(a,b)$: the proportion of external infections is assumed to follow a Beta distribution of fixed parameters (e.g. $p(\pi) = \beta(1.5,10)$ to allow for around 10\% of external infections).	
\end{itemize}
