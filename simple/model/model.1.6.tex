...
We note $\kappa = \{\kappa_1, \ldots, \kappa_n\}$.
	...
	\item $\phi$: an hyperparameter corresponding to the probability for an observed case to have been imported from the outside, used to define the priors on $\alpha_i$.
\end{itemize}
...
\begin{eqnarray}
& & p(\{s_i, t_i, T_i^{inf}\}_{(i=1,\ldots,n)}, w,  \alpha, \kappa, \mu_1, \gamma, \pi, \phi)\\
& = & p(\{s_i, t_i, T_i^{inf}\}_{(i=1,\ldots,n)}| w,  \alpha, \kappa, \mu_1, \gamma, \pi, \phi) \times p( w,  \alpha, \kappa, \mu_1, \gamma, \pi, \phi)
\end{eqnarray}
where the first term is the likelihood of observed and augmented data, and the second, the prior.
The likelihood can be decomposed as (the hyperparameters $\pi$ and $\phi$ are dropped as they only affect the priors on $\kappa$ and $\alpha$):
...
\begin{eqnarray}
\Omega_i^2 & = & p(t_i | T_i^{inf}, w) = f_w(t_i - T_i^{inf})
\end{eqnarray}
\\
...
\begin{itemize}
	...
	\item $p(\alpha_i) = \mathbf{1}_{\{k=0\}}\phi +  \mathbf{1}_{\{k\neq i, k>0\}}\frac{1-\phi}{n-1}$, where $\phi$ is the probability of cases imported from the outside. $\phi$ will be assumed to follow a Beta distribution of fixed parameters (e.g. $p(\pi) = \beta(1.5,10)$ to allow for around 10\% of external infections).
% 	\item $p(\alpha_i = 0) = \phi$, by definition. For $\alpha_i>0$, $p(\alpha_i = k) = \mathbf{1}_{\{k\neq i\}}\frac{1-\phi}{n-1}$.
% 	\item $p(\alpha_i) = \mathbf{1}_{\{k\neq i\}}\frac{1}{n-1}$.
	\item $p(\kappa_i - 1) = \mathcal{NB}(1,1-\pi)$, the probability mass function of a negative binomial distribution counting the number of unobserved cases before one observed case; $1-\pi$ is the proportion of unobserved (unsampled) cases in the outbreak. If we assume that the entire outbreak has been sampled, $\pi=1$ and $p(\kappa_i) = \mathbf{1}_{\left\lbrace \kappa_i=1\right\rbrace}$. Otherwise, $\pi$ will be assumed to follow a Beta distribution of fixed parameters (e.g. $p(\pi) = \beta(10,1)$ for an a priori densely sampled outbreak).
