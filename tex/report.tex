...
\chapter{Results}
We performed initial testing on the model by seeing how well the model performs when reproducing parameters from a dataset generated by the outbreak simulation process {\tt simOutbreak}. The model was fitted to randomly generated datasets with epidemiological parameters chosen to represent the real properties of different pathogens. The first imagined scenario is to try and infer the spatial framework from simulated case data of the West African Ebola virus disease (EVD), it is speculated that the dynamics of the recent outbreak revolved heavily around the free and regular movement of people between large towns in the effected region. We can attempt to reproduce this situation by grouping the data depending on the home town of the individual, if we make the assumption that people from each town regularly visit other towns nearby depending on how close they are and that transmission between towns happens when these visits occur then we can generate case data using a transmission matrix where the transmission probabilities reflect the amount of visitors there are between two towns. If more people visit town A who come from town B than town C then as a person living in town A I expect that if I am infected with EVD then there is a higher probability that my infector is from town B than from town C and that I am much more likely to be infected by someone from my own town due to the relative frequencies of people from each town that I will come into contact with (assuming that there is random mixing within each town).
...
\section{Emulating the Ebola outbreak}
