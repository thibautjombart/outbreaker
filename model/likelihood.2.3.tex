...
% 	\item $\alpha$ the probability that two sampled isolates belong to the same lineage (i.e., one is the direct ancestor of the other).
	\item $\alpha$ the probability of having sequenced a given lineage existing within a patient; all lineages within a host are assumed to be equiprobable, so that $\alpha = 1/\mbox{nb lineages}$.
\end{itemize}
...
The genetic likelihood given the infection $i \leftarrow j$ relies on how likely it is that a sequence in $S_i$ has an ancestor (i.e., an isolate from the same lineage) in $S_j$.
...
% p(s_i^k | s_i^1, \ldots, s_i^{k-1}, S_j, i \leftarrow j) & = & b_{s_i^k, j} (\Xi_{s_i^k}^{(j)} + \Xi_{s_i^k}^{(i)}) + (1-b_{s_i^k, j}) \\
p(s_i^k | S_j, i \leftarrow j) & = & \mathcal{B}\left(1 \lvert \sum_{j=1}^{m_j}\mathbf{1}_{\{t_i^k \geq t_j^q\}}, \alpha\right) \Xi^{(j)} + 1 - \mathcal{B}\left(1 \lvert \sum_{j=1}^{m_j}\mathbf{1}_{\{t_i^k \geq t_j^q\}}, \alpha\right) \\
\end{eqnarray*}
with:
\begin{itemize}
\item $\mathcal{B}(.\lvert n,p)$: probability mass function of the Binomial distribution with $n$ draws and probability $p$.
\item $\sum_{j=1}^{m_j}\mathbf{1}_{\{t_i^k \geq t_j^q\}}$: number of sequences in patient $j$ sampled before the sequence $s_i^k$.
\item $\Xi^{(j)}$: direct ancestries to $s_i^k$ from sequences in patient $j$ ($S_j$).
%  \item $b_{s_i^k,j}$: equals 1 if a direct ancestor to sequence $s_i^k$ exists among $(S_i,S_j)$, and 0 otherwise;  
%  \item $\Xi_{s_i^k}^{(j)}$: direct ancestries to $s_i^k$ from sequences in patient $j$ ($S_j$)
%  \item $\Xi_{s_i^k}^{(i)}$: direct ancestries to $s_i^k$ from sequences in patient $i$ ($S_i$)
...
$\Xi^{(j)}$ is given by:
\begin{eqnarray*}
\Xi_{s_i^k}^{(j)} & = & \sum_{q=1}^{m_j} \left(\mathcal{P}\left(d_{s_i^k,s_j^q} | \nu_1 \mathbf{1}_{\{t_i^k \geq t_j^q\}}(t_i^k - t_j^q) l_{s_i^k,s_j^q}\right)
\times \mathcal{P}\left(g_{s_i^k,s_j^q} | \nu_2 \mathbf{1}_{\{t_i^k \geq t_j^q\}}(t_i^k - t_j^q) l_{s_i^k,s_j^q}\right) \right) \\
% \Xi_{s_i^k}^{(i)} & = & \sum_{r=1}^{k-1} \left(\mathcal{P}\left(d_{s_i^k,s_i^r} | \nu_1 (t_i^k - t_i^r) l_{s_i^k,s_i^r}\right)
% + \mathcal{P}\left(g_{s_i^k,s_i^r} | \nu_2 (t_i^k - t_i^r) l_{s_i^k,s_i^r}\right) \right) \\
...
\begin{itemize}
	...
	\item $\alpha$ can be parameterized using two approaches: i) using a prior on the number of lineages within a single host, and taking $\alpha=1/\mbox{nb. lineages}$ ii) inferred from the numbers of identical haplotypes sampled within a host at a given time point.
\end{itemize}
