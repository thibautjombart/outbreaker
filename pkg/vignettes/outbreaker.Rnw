...
\begin{abstract}
  ...
  visualization and interpretation of results. For an introduction to disease outbreak
  reconstruction, see online tutorials available on the \textit{R-epi project}:
...
<<>>=
anova(lm(post~run, data=x[x$step>2e4,]))
@
\\
...
plot(g, edge.curved=FALSE, vertex.color=case.color$col, layout=layout.auto, vertex.size=case.size, edge.label="")
title("Consensus ancestries")
legend("bottomleft", col=case.color$leg.col, leg=case.color$leg.txt, title="Mean infection date", pch=20, pt.cex=3, inset=-.1)
@
...
<<>>=
boxplot(Tinf, horizontal=TRUE, las=1, xlab="Date",
...
<<>>=
p + geom_boxplot(aes(x=run, y=pi, fill=run),alpha=.6) + geom_jitter(aes(x=run, y=pi, col=run),alpha=.4)
@
...
abline(v=quantile(mu, c(.025, .5, .975)), lty=c(2,1,2), lwd=2, col="royalblue")
legend("topright", lty=c(1,2), leg=c("Median","95% credibility interval"), col="royalblue", bg="white")
@
...
<<>>=
p + geom_hex(bins=8) + geom_smooth(colour="red",size=1)
@
~\\
...
table(R)
table(R)/length(R)
barplot(table(R)/length(R), xlab="Number of secondary cases", main="Posterior estimates of effective reproduction numbers (R)", ylab="Frequency")
@
...
p <- ggplot(data=x, aes(x=case, y=R)) + labs(title="Posterior estimates of effective reproduction numbers")
p + geom_boxplot(aes(colour=case)) + guides(colour=FALSE)
