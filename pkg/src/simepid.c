...
int SimulEpid(parameters *param, hospDurationParam *paramHosp, nb_data *nbData, raw_data *data, aug_data *augData, int *indexInfector)
{
    ...
    /* fill in the nb of patients with a least one positive swab */
    for(i=0;i<NbCases;i++){
	if(nbData->NbPosSwabs[i]>0){
	    nbData->indexColonisedPatients[nbData->NbColonisedPatients] = i;
	    nbData->NbColonisedPatients = nbData->NbColonisedPatients + 1;
	}
    }

    printf("\nnbData->NbColonisedPatients: %d\n", nbData->NbColonisedPatients);
    fflush(stdout);
